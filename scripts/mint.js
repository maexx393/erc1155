...
const NUM_ANIMALBOXES = 4
const DEFAULT_OPTION_ID = 0
const ANIMALBOX_OPTION_ID = 2
...
async function main() {
    ...
    if (NFT_CONTRACT_ADDRESS) {
        ...
        // aanimalboxes issued directly to the owner.
        for (var i = 0; i < NUM_ANIMALBOXES; i++) {
            const result = await factoryContract.methods.mint(ANIMALBOX_OPTION_ID, OWNER_ADDRESS).send({ from: OWNER_ADDRESS });
            console.log("Minted animalbox. Transaction: " + result.transactionHash)
        }
    }
}
