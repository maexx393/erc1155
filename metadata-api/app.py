...
def box(token_id):
    token_id = int(token_id)
    image_url = _compose_image(['images/box/animalbox.png'], token_id, "box")

    attributes = []
    _add_attribute(attributes, 'number_inside', [3], token_id)

    return jsonify({
        'name': "Creature Animal Box",
        'description': "This animalbox contains some OpenSea Creatures! It can also be traded!",
        'image': image_url,
        'external_url': 'https://openseacreatures.io/%s' % token_id,
        'attributes': attributes
    })
...
def factory(token_id):
    ...
    if token_id == 0:
        ...
        name = "One OpenSea creature animalbox"
        description = "When you purchase this option, you will receive one animalbox, which can be opened to reveal three " \
                      "OpenSea creatures of random variety. Enjoy and take good care of these cute aquatic beings!"
        image_url = _compose_image(['images/box/animalbox.png'], token_id, "factory")
        num_inside = 3
